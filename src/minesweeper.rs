...
impl Minesweeper {
    ...
    // function for placing the mines, uses rand for randomizing the locations of the mines
    fn place_mines(grid: &mut Vec<Vec<Cell>>, num_bombs: usize) {
        ...
        for (i, j) in cells.into_iter().take(num_bombs) {
            grid[i][j].set_has_mine();
        }
    }

    // function for creating the grid for the game, creates the required 10x10 grid with 10 bombs
    pub fn create_grid(size: usize, bombs: usize) -> Vec<Vec<Cell>> {
        ...
        Minesweeper::place_mines(&mut grid, bombs);
        grid
    }

    // calculates the amount of adjacent mines of a guessed cell
    ...
    // function for guessing a cell, uses recursion for opening up the neighbors of a cell with no adjacent bombs
...
// visual representation of the game as a string
...
// test for creating the grid and game + using core functions
#[cfg(test)]
mod tests {
    ...
    #[test]
    fn test() {
        let grid = Minesweeper::create_grid(10,15);
